...
def bounding_box_filter(cloud_ndarray, heap_area, save_url):
    ...
    for p in heap_area:
        poly_area.append([p.x * 100, p.y * 100])
        x_list.append(p.x * 100)
        y_list.append(p.y * 100)

    # 多边形的起始点即终点
    origin_x = heap_area[0].x * 100
    origin_y = heap_area[0].y * 100
